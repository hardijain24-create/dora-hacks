...
if __name__ == '__main__':
    ...
    for entry in manifest:
        ...
        for seg in tqdm(entry['segments']):
            start_sec = seg.get('start', 0.0)
            end_sec = seg.get('end', 0.0)
            label = seg.get('label', 'UNKNOWN')
            
            # We will extract even UNCERTAIN segments so we can train a placeholder model
    ...
    if len(all_data) > 0:
        np.save(os.path.join(out_dir, 'X.npy'), np.array(all_data))
        with open(os.path.join(out_dir, 'y.json'), 'w') as f:
            json.dump(all_labels, f)
        print(f"Extraction complete! Saved {len(all_data)} sequences.")
    else:
        print("No valid segments found. Please ensure the manifest has actual segments and labels (not UNCERTAIN).")
